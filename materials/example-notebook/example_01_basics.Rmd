...
# Set BRMS backend FIRST before loading brms
# cmdstanr should be pre-compiled in container
if (requireNamespace("cmdstanr", quietly = TRUE)) {
  tryCatch({
    cmdstanr::cmdstan_path()
    options(brms.backend = "cmdstanr")
    cat("✓ Using CmdStan backend (pre-compiled)\n")
  }, error = function(e) {
    cat("! CmdStan not found, using RStan\n")
  })
} else {
...
if (requireNamespace("cmdstanr", quietly = TRUE)) {
  ...
  if (inherits(cmdstanr_path, "try-error")) {
    ...
    cat("✓ CmdStan found at:", cmdstanr_path, "\n")
  }
} else {
...
tryCatch({
  library(rstan)       # Load rstan first for compatibility
  library(brms)        # Bayesian Regression Models using Stan
  library(tidyverse)   # Data manipulation
  library(bayesplot)   # Posterior visualization
  library(ggplot2)     # Plotting
  cat("✓ All packages loaded successfully\n")
  cat("✓ BRMS backend:", getOption("brms.backend", "rstan"), "\n")
}, error = function(e) {
...
1. ✅ Loading data and fitting a Bayesian regression model
2. ✅ Understanding posterior distributions
3. ✅ Visualizing results
4. ✅ Specifying custom priors
5. ✅ Comparing models with LOO
