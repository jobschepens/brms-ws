...
run_cont_simulation <- function(n_sim = 20, true_effect = 0, noise_sd = 0.2, cache_prefix = "sim_cont", brm_model = NULL) {
  ...
  for (i in 1:n_sim) {
    ...
    for (n in points_to_run) {
      # Need to ensure sequential order if 'stopped' logic matters?
      # points_to_run is sorted? setdiff might unsort.
      # sort it.
    }
    ...
    for (n in points_to_run) {
      ...
      if (!is.null(brm_model)) {
        ...
        if (!dir.exists("models/cache_fits")) dir.create("models/cache_fits", recursive=TRUE)
