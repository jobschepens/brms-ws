...
#| fig-cap: "Savage-Dickey Density Ratio Illustration"
...
- Prior density at θ = 0: `r round(prior_at_0, 3)`
- Posterior density at θ = 0: `r round(posterior_at_0, 3)`
- The posterior density **decreased** at the null
- BF₁₀ = `r round(BF_10, 2)` → Data are ~`r round(BF_10, 1)` times more likely under H₁
...
```{r}
#| label: hypothesis-test-complexity
...
```{r}
#| echo: false
# Extract values from the hypothesis test
h_summary <- h_test$hypothesis
cat("Hypothesis:", h_summary$Hypothesis, "\n")
cat("Estimate:", sprintf("%.2f", h_summary$Estimate), "\n")
cat("CI.Lower:", sprintf("%.2f", h_summary$CI.Lower), "\n")
cat("CI.Upper:", sprintf("%.2f", h_summary$CI.Upper), "\n")
cat("Evid.Ratio:", sprintf("%.2f", h_summary$Evid.Ratio), "\n")
cat("Post.Prob:", sprintf("%.2f", h_summary$Post.Prob), "\n")
cat("Star:", h_summary$Star, "\n")
```

**Interpretation:**

- **Estimate**: Complex sentences are `r sprintf("%.2f", abs(h_summary$Estimate))` log-units slower
- **95% CI**: [`r sprintf("%.2f", h_summary$CI.Lower)`, `r sprintf("%.2f", h_summary$CI.Upper)`] (doesn't include 0)
- **Evid.Ratio ($BF_{01}$)**: The output `Evid.Ratio` for a point null test ("= 0") represents the **Bayes Factor for the Null**.
  - Value: `r sprintf("%.4f", h_summary$Evid.Ratio)`
...
- **Bayes Factor ($BF_{10}$)**: To get evidence **for the effect**, we take the inverse ($1 / BF_{01}$):
  - $BF_{10} = 1 / `r sprintf("%.5f", h_summary$Evid.Ratio)` \approx `r format(1/h_summary$Evid.Ratio, digits=3, scientific=TRUE)`$
- **Conclusion**: The data provide **decisive evidence** for a complexity effect ($H_1$) over the null hypothesis ($H_0$).

### Directional Test

```{r}
#| label: directional-hypothesis-test
...
- **Estimate**: `r sprintf("%.2f", abs(h_directional$hypothesis$Estimate))` (log-units)
- **Post.Prob**: `r sprintf("%.2f", h_directional$hypothesis$Post.Prob)` (1.00) means we are 100% certain the effect is negative ($< 0$).
...
```{r}
#| label: example-group-comparison
...
```{r}
#| label: rope-integration-example
...
- **Estimate**: Native speakers are `r sprintf("%.2f", h_group_diff$hypothesis$Estimate)` log-odds (ns) different.
- **Evid.Ratio ($BF_{01}$)**: `r sprintf("%.2f", h_group_diff$hypothesis$Evid.Ratio)` (1.16).
...
```{r}
#| label: fig-bf-scale
#| fig-cap: "Interpreting Bayes Factors"
...
val_bf <- bf_models$bf
favor_rs <- val_bf > 1
direction <- if(favor_rs) "random slopes (RS)" else "random intercepts (RI)"
magnitude <- if(favor_rs) val_bf else 1/val_bf
```

- **Bayes Factor ($BF_{RS, RI}$)**: `r sprintf("%.2f", val_bf)`
- **Conclusion**: The data provide evidence **in favor of the `r direction` model** by a factor of `r sprintf("%.2f", magnitude)`.
...
```{r}
#| label: create-bf-comparison-table
...
```{r}
#| label: check-bridge-convergence
...
```{r}
#| label: visualize-joint-distribution
#| fig-width: 8
#| fig-height: 6
...
```{r}
#| label: compare-informed-default-priors
...
```{r}
#| label: sequential-testing-example
#| eval: true
