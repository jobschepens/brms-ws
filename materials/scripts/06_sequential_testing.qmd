...
2.  **Small N (20)**:
    *   **BF**: Likely inconclusive or favoring Null (especially with wide prior).
    *   **ROPE**: Likely "Undecided" (Posterior is wide, overlapping both 0 and ROPE).
    *   **LOO**: Often prefers the simpler Null model (Occam's razor via overfitting penalty).
3.  **Large N (100)**:
    *   **BF**: Starts to strongly favor H1 (if effect is real).
    *   **ROPE**: Posterior tightens. If effect is 0.2 and ROPE is [-0.1, 0.1], 0% should be in ROPE -> "Reject Null".
    *   **LOO**: Starts to recognize H1 predicts better.

# Part 2: Directional Hypothesis

Now let's look at a continuous predictor $X$ and a directional hypothesis $X > 0$.

```{r}
#| label: directional-setup
...
fit_cont <- brm(y ~ x, data = df_cont, refresh = 0)
```

We test the hypothesis that the slope > 0.

## Using `hypothesis()`

```{r}
#| label: directional-hypothesis
hyp <- hypothesis(fit_cont, "x > 0")
print(hyp)
```

The `Evid.Ratio` here represents $P(H_1) / P(H_0)$. For a one-sided hypothesis, this is simply $P(\beta > 0) / P(\beta \le 0)$.

## Using `marginaleffects` and `bayestestR`

We can inspect the entire distribution to see *how much* of it is greater than 0, and combined with a ROPE.

```{r}
#| label: directional-viz
#| fig.width: 8
#| fig.height: 5
...
*   **Probability of Direction (pd)**: The % of the posterior that has the same sign as the median. If $pd = 99\%$, it means 99% of the posterior is positive.
*   **ROPE**: Even if $pd$ is high (sure it's positive), the effect might still be inside the ROPE (practically negligible).

Checking these two together gives a complete picture: "We are sure it is positive ($pd \approx 100\%$), and we are sure it is practically significant (0% in ROPE)."
